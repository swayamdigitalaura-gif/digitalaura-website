...
const platforms = [
  {
    ...
    rating: "4.9",
    reviews: "50+",
    tagline: "Best of Clutch · Digital Marketing 2025",
    color: "#E8251A",
  },
  {
    name: "GoodFirms",
    logo: "/logos/goodfirms.png",
    rating: "4.8",
    reviews: "40+",
    tagline: "Top Digital Marketing Company",
    color: "#2E86DE",
  },
  {
    name: "DesignRush",
    logo: "/logos/designrush.webp",
    rating: "4.7",
    reviews: "30+",
    tagline: "Best Digital Marketing Agencies",
    color: "#6C47FF",
  },
  {
    name: "Google",
  ...
  <section className="py-20 px-4 md:px-8" style={{ background: "#FFFFFF" }}>
    <div className="max-w-5xl mx-auto">
      ...
      >
        ...
        <p className="text-[#6B7280] mt-3 text-sm max-w-md mx-auto">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-5">
        {platforms.map((p, i) => (
          ...
          >
            {/* Logo area — image centered, tagline always at bottom */}
            <div
              className="w-full flex flex-col items-center px-4 pt-5 pb-4"
              style={{ height: 170 }}
            >
              {/* Image fills remaining space, centered vertically */}
              <div className="flex-1 flex items-center justify-center">
                {p.logo === "google" ? (
                  ...
                  <img src={p.logo as string} alt={p.name} className="object-contain"
                    style={{ height: 85, maxWidth: 150, mixBlendMode: "multiply" }} />
                )}
              </div>
              {/* Tagline pinned at bottom — same position on every card */}
              <span className="text-[10px] font-semibold px-3 py-1.5 rounded-full text-center leading-snug"
                style={{ background: `${p.color}12`, color: p.color, maxWidth: "90%" }}>
                {p.tagline}
              </span>
            </div>
            ...
            <div className="w-full px-5 pb-5 border-t pt-4" style={{ borderColor: "#F3F4F6" }}>
              <div className="flex gap-0.5 justify-center mb-2">
                {[...Array(5)].map((_, j) => (
                  <Star key={j} size={13} fill={p.color} stroke="none" />
                ))}
              </div>
...
export default RatedOnPlatforms;
