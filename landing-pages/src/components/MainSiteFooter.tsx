...
function LinkItem({ label, href, dot }: { label: string; href: string; dot: string }) {
  return (
    <li>
      <a
        href={href}
        className="group text-[13px] text-white hover:opacity-80 transition-opacity duration-200 flex items-center gap-2"
      >
        <span
  ...
  return (
    <footer style={{ background: "#070E1C" }} className="relative overflow-hidden">
      <div
        ...
        style={{
          background: "linear-gradient(90deg, transparent, #7C3AED, #FF6B2B, #1A6FE8, transparent)",
        }}
      />
      <div
      ...
      />
      <div
        className="absolute top-0 left-1/4 w-80 h-80 rounded-full pointer-events-none opacity-[0.05]"
        style={{
      ...
      />
      <div
        className="absolute bottom-0 right-1/4 w-64 h-64 rounded-full pointer-events-none opacity-[0.04]"
        style={{
      ...
      <div className="max-w-7xl mx-auto px-4 md:px-8 pt-12 pb-6 relative z-10">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8 mb-8">
          ...
          <div>
            ...
            <ul className="grid grid-cols-2 lg:grid-cols-1 gap-x-2 gap-y-2.5">
              {quickLinks.map((l) => (
                <LinkItem key={l.label} {...l} dot="#FF6B2B" />
              ))}
            </ul>
          </div>
                ...
                {servicesLeft.map((group) => (
                  <div key={group.heading}>
                    ...
                    <ul className="space-y-2">
                      {group.items.map((l) => (
                        <LinkItem key={l.label} {...l} dot={group.color} />
                      ))}
                    </ul>
                  </div>
                ))}
          ...
          <div>
            ...
            <div className="flex gap-2.5">
              <a
                href="https://www.linkedin.com/company/thedigitalaura/"
                target="_blank"
                rel="noopener noreferrer"
                aria-label="LinkedIn"
              ...
              <a
                href="https://www.instagram.com/thedigitalaura/"
                target="_blank"
                rel="noopener noreferrer"
        ...
        <div className="flex flex-col sm:flex-row items-center justify-between gap-3 text-[11px] text-white">
          ...
          <div className="flex items-center gap-4">
            ...
            <a href="/cancellation-refund-policy" className="hover:opacity-70 transition-opacity">
