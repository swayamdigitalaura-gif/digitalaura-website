  ...
  >
    ...
    <div className="border-t border-gray-100 bg-[#FAFAFA]">
      <div className="max-w-7xl mx-auto px-4 md:px-6 py-3 flex items-center justify-between">
        ...
        <a href={`${BASE}/services`} onClick={onClose} className="inline-flex items-center gap-1.5 text-[12px] font-bold text-[#FF6B2B] hover:gap-2.5 transition-all">
    ...
    <>
      ...
      >
        <div className="max-w-7xl mx-auto flex items-center justify-between h-[72px] px-4 md:px-6">
          ...
          <a href={BASE} className="shrink-0 flex items-center gap-2">
            <img src={logo} alt="Digital Aura" className="h-[82px] w-auto object-contain" style={{ mixBlendMode: "multiply" }} />
          </a>
          ...
          <div className="hidden lg:flex items-center gap-4">
            ...
            {["Case Studies", "Blogs", "Contact"].map((label) => {
              const href = label === "Case Studies" ? `${BASE}/case-studies` : label === "Blogs" ? `${BASE}/blog` : `${BASE}/contact`;
              return (
                <a key={label} href={href} className="text-[14px] font-medium text-[#374151] hover:text-[#FF6B2B] transition-colors relative group whitespace-nowrap">
                  {label}
                  <span className="absolute -bottom-0.5 left-0 h-0.5 rounded-full transition-all duration-300 bg-[#FF6B2B] w-0 group-hover:w-full" />
                </a>
              );
          ...
          <div className="hidden lg:flex items-center gap-3 shrink-0">
            ...
            <a href={`${BASE}/contact`} className="btn-orange px-5 py-2.5 text-[15px]">Get Growth Plan</a>
          </div>
      ...
      >
        ...
        <div className="flex flex-col px-6 flex-1">
          ...
          <div>
            ...
            <div className={`overflow-hidden transition-all duration-200 ${mobileCompanyOpen ? "max-h-60 opacity-100" : "max-h-0 opacity-0"}`}>
              <div className="py-2 space-y-1">
                {companyLinks.map((cl) => (
                  <a key={cl.label} href={cl.href} onClick={() => setMobileOpen(false)}
          ...
          <div>
            <div className="flex items-center justify-between border-b border-gray-100 py-4">
              ...
              <button onClick={() => setMobileServicesOpen(v => !v)} className="p-2 -mr-2 text-[#0A1628]">
            ...
            <div className={`overflow-hidden transition-all duration-200 ${mobileServicesOpen ? "max-h-[600px] opacity-100" : "max-h-0 opacity-0"}`}>
              {megaCategories.map(cat => (
                <div key={cat.key} className="mb-4 mt-2">
                  ...
                  {cat.items.map(item => (
                    <a key={item.title} href={item.route} onClick={() => setMobileOpen(false)}
        ...
        <div className="px-6 pb-8 mt-4 space-y-3">
          ...
          <a href={`${BASE}/contact`} className="btn-orange px-6 py-4 text-base w-full block text-center">
            Get Growth Plan →
          </a>
        </div>
      </div>
